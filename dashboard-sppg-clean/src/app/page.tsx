...
export default function Dashboard() {
  ...
  const filteredData = useMemo(() => {
    const filtered = data.filter(row => {
      const matchesSearch = 
        row.namaSppg.toLowerCase().includes(searchTerm.toLowerCase()) ||
        row.kepalaSppg.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const generateRowText = () => {
    return `${formData.tanggal}\t${formData.namaSppg}\t${formData.kepalaSppg}\t${formData.noTelephone}\t${formData.teknisi}\t${formData.statusBa}\t${formData.statusPekerjaan}\t${formData.statusPembayaran}\t${formData.keterangan}`
  }
  ...
  const copyEditToClipboard = async () => {
    const rowText = `${editFormData.tanggal}\t${editFormData.namaSppg}\t${editFormData.kepalaSppg}\t${editFormData.noTelephone}\t${editFormData.teknisi}\t${editFormData.statusBa}\t${editFormData.statusPekerjaan}\t${editFormData.statusPembayaran}\t${editFormData.keterangan}`
    await navigator.clipboard.writeText(rowText)
    setCopied(true)
    setTimeout(() => setCopied(false), 2000)
  }
                      ...
                      <Table>
                        <TableHeader className="sticky top-0 bg-white dark:bg-slate-900 z-10">
                          <TableRow>
                            ...
                            <TableHead className="min-w-[150px]">Nama SPPG</TableHead>
                            <TableHead className="min-w-[130px]">Kepala SPPG</TableHead>
                          ...
                          {filteredData.map((row, index) => {
                            ...
                            return (
                              <TableRow key={index} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                ...
                                <TableCell className="font-medium whitespace-nowrap">{row.namaSppg}</TableCell>
                                <TableCell className="whitespace-nowrap">{row.kepalaSppg}</TableCell>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor="add-namaSppg">Nama SPPG</Label>
                  <Input
                    id="add-namaSppg"
                    placeholder="Nama SPPG"
                    value={formData.namaSppg}
                    onChange={(e) => handleInputChange('namaSppg', e.target.value)}
                    required
                  />
                </div>

                <div className="space-y-2">
                  <Label htmlFor="add-kepalaSppg">Kepala SPPG</Label>
                  <Input
                    id="add-kepalaSppg"
                    placeholder="Nama Kepala SPPG"
                    value={formData.kepalaSppg}
                    onChange={(e) => handleInputChange('kepalaSppg', e.target.value)}
                    required
                  />
                </div>
        ...
        <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="edit-namaSppg">Nama SPPG</Label>
                <Input
                  id="edit-namaSppg"
                  value={editFormData.namaSppg}
                  onChange={(e) => handleEditInputChange('namaSppg', e.target.value)}
                />
              </div>

              <div className="space-y-2">
                <Label htmlFor="edit-kepalaSppg">Kepala SPPG</Label>
                <Input
                  id="edit-kepalaSppg"
                  value={editFormData.kepalaSppg}
                  onChange={(e) => handleEditInputChange('kepalaSppg', e.target.value)}
                />
              </div>
